...
export default function LikedVideosPage() {
  ...
  useEffect(() => {
    ...
    // In a real app, we'd fetch videos liked by the user
    // For now, let's just show some videos as a placeholder
    getVideos({ sortBy: 'views', excludeShorts: true }).then((data) => {
      if (!cancelled) {
        setVideos(data.slice(0, 5)); // show top 5 as "liked"
        setLoading(false);
      }
    });
    ...
    <div className="min-h-screen p-4 sm:p-8 max-w-screen-2xl mx-auto pb-16">
      ...
      <div className="flex items-center gap-4 mb-8 border-b border-border pb-6">
        <div className="w-16 h-16 rounded-full bg-surface-tertiary flex items-center justify-center">
          <ThumbsUp className="w-8 h-8 text-brand" />
        </div>
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-text-primary">Liked Videos</h1>
          <p className="text-text-muted mt-1">Videos you've liked</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 gap-4 text-text-muted animate-fade-in">
          <ThumbsUp className="w-16 h-16 opacity-40" />
          <p className="text-lg font-medium">No liked videos yet</p>
          <p className="text-sm">Tap on like icon under a video to save it here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-8">
          {videos.map((video) => (
